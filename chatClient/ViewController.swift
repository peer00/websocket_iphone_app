...
class ViewController: UIViewController, WebSocketDelegate {
    var socket = WebSocket(url: NSURL(scheme: "ws", host: "joel.princesspeach.nyc:3000", path: "/")!, protocols: ["chat", "superchat"])
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        socket.delegate = self //a variable that conforms to a protocol
        socket.connect()
    }
